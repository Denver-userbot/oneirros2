...
class RivalRegionsAuthedSpider < Kimurai::Base
  ...
  def self.authed_parse!()
    ...
    return self.parse!(:authed_callback, url: "http://rivalregions.com/?viewer_id=#{rivals_id}&id=#{rivals_id}&access_token=#{rivals_token}&hash=#{rivals_hash}")
  end
  ...
  def authed_callback(response, url:, data: {})
    # Redirect to implementing class
